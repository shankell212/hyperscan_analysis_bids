...
%% Set paths 
baseDirectory = '/projectnb/nphfnirs/s/datasets/Hyperscanning_controls_2425/';

dyad_num =  '17' %'08'%'07'; % CHANGE (dyad number)

p1 = "sub-33" % CHANGES FOR EACH DYAD PAIR - subject IDs
p2 = "sub-34" %
...
tasks = ["IR", "JR", "JS","PN"]%,"PN"]%, "PN"] %, "PN"];
runNums_lst = ["01","01","01","01"]%,"02"]%, "02"];
%run_num = '01';
...
if PREPROCESS
    ...
    results_dir = char(join([baseDirectory, 'derivatives/', ses, '/'], ''));
    path1 = char(join([results_dir, p1, "_data_", ses], ''));
    load(path1)
    path2 = char(join([results_dir, p2, "_data_", ses], ''));
    load(path2)
...
%for k= 2
    for i=1:length(groupRegion)
        ...
        elseif all(isnan(u_subj1_HbO(:,2)), 'all') || all(isnan(u_subj2_HbO(:,2)), 'all')
            ...
            [coherence,period,scale, coi, t] = wtc(u_subj1_HbO,u_subj2_HbO);
...
disp('Done with WTC calc')

%%
% results = char(join([baseDirectory, 'derivatives/', ses, '/'], ''));
% cd(results);
% load(char(join(["WTC_GroupRegion_", ses, ".mat"],'')), "WTC");
...
% cd(plot_dir);
% 
% task = {'joint singing', 'separate reading', 'joint reading','picture guessing'}; %, 'convo'};
% 
% % go through all WTC plot
% for k=2:length(task) % 4 tasks
